        ...
        if (data.rpcName === "get_dashboard_summary") {
          ...
          const currentIndicators = await query(
            `SELECT 
              ...
              COALESCE(SUM(CASE WHEN type::text IN ('income', 'extra_income') AND status = 'pending' THEN amount ELSE 0 END), 0) as to_receive,
              COALESCE(SUM(CASE WHEN type::text IN ('expense', 'fixed_expense') AND status = 'pending' THEN amount ELSE 0 END), 0) as to_pay
             FROM public.transactions 
          ...
          const alerts = await query(
            `SELECT 
              ...
              COUNT(CASE WHEN status = 'pending' AND type::text IN ('expense', 'fixed_expense') AND due_date BETWEEN $2 AND (CURRENT_DATE + interval '7 days') THEN 1 END) as soon_count
             FROM public.transactions 
